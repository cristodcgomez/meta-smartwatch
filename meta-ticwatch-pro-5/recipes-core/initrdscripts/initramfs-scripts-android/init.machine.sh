...
# Note: Partition numbers may need adjustment for TicWatch Pro 5
mkdir -m 0777 $BOOT_DIR/boot
mount -t auto -o ro /dev/mmcblk0p33 $BOOT_DIR/boot
...
if [ ! -e $BOOT_DIR/etc/asteroid/machine.conf ] ; then
    ...
    echo dace > /sys/kernel/config/usb_gadget/g1/strings/0x409/product
