...
# MACHINE: meta-dace is in bblayers for every machine (update_layer_config
# auto-includes all meta-smartwatch sublayers), and an unconditional dep
# would make every other machine's asteroid-image demand dace-boot-images
# (COMPATIBLE_MACHINE-skipped there) and fail.
do_image_complete[depends] += "${@bb.utils.contains('MACHINE', 'dace', 'dace-boot-images:do_build', '', d)}"
