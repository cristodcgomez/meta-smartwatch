FILESEXTRAPATHS:prepend:dace := "${THISDIR}/usb-moded:"

# Dace has no /sys/class/android_usb interface -- only the configfs gadget
...
#    `-`-tolerated and dace doesn't ship init_ffs, and cable state comes
...
#    because adb is dace's primary debug channel. The default is shipped as
#    /etc/usb-moded/dace-defaults.ini -- NOT as /etc/usb-moded/usb-moded.ini
...
#    in our 99-dace-no-configfs.conf drop-in to android-tools). It
...
#
# 8. 10-dace-android-tools.preset disables the upstream auto-enable of
...
do_install:append:dace() {
    ...
    # meta-dace/recipes-devtools/android-tools/files/
    # 99-dace-no-configfs.conf). Do NOT add a basic.target.wants symlink here
    ...
    install -m 0644 ${UNPACKDIR}/10-dace-android-tools.preset \
        ${D}${systemd_unitdir}/system-preset/10-dace-android-tools.preset
    # /etc/usb-moded/dace-defaults.ini: matched by the static-config glob
    # /etc/usb-moded/*.ini and NOT the legacy delete-on-first-boot path (which
    # only targets /etc/usb-moded/usb-moded.ini).
    install -m 0644 ${UNPACKDIR}/dace-defaults.ini \
        ${D}/etc/usb-moded/dace-defaults.ini
    ...
    # so dynamic = developer_mode would override our dace-defaults.ini static
...
FILES:${PN}:append:dace = " ${systemd_unitdir}/system/init_gfs.service \
                              ${systemd_unitdir}/system/sysinit.target.wants/init_gfs.service \
                              ${systemd_unitdir}/system/adbd-prepare.service \
                              ${systemd_unitdir}/system-preset/10-dace-android-tools.preset"
