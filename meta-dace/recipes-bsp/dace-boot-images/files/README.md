# dace-boot-images source files

This directory holds the per-port static inputs that
`dace-boot-images.bb` consumes to produce `boot.img`,
...
`dace-vkb-modules.lst`. KMI-CRC-checks every module against the
kernel's `Module.symvers` so we catch ABI breaks at build time.

### `dace-vkb-modules.lst`

Manifest of `.ko`'s that go into `vendor_kernel_boot.img`. Each entry
is `<name>:k` (kernel-built, from linux-dace's kernel-modules
package) or `<name>:tp` (techpack-built, from the linux-dace-modules
...
early boot (what *loads* is init_boot's `modules.load.dace`).
...
1. `dace-vkb-modules.lst` — rebuild the manifest from the new
...
5. Adjust `RAMOOPS_BASE`/`SIZE` in `dace-boot-images.bb` to match
   the new device's reserved-memory layout
6. Adjust `MKBOOTIMG_*` offsets if the new SoC uses a different
   memory map

`dace-vkb-assemble.py` itself is board-agnostic and shouldn't need
porting.
