SUMMARY = "Pixel Watch 2 (dace) Qualcomm/Google vendor kernel modules"
...
linux-dace with the same clang + config, so vermagic/CRC/ABI match \
...
PV = "${DACE_KERNEL_VERSION}"
PKGV = "${DACE_KERNEL_VERSION}"
...
# socmsm is pinned to the PIXEL-WATCH branch (dace variant) — its
...
# Build against the exact linux-dace kernel (same clang + config → matching
...
# linux-dace uilds out-of-tree (B != S): kernel SOURCE in STAGING_KERNEL_DIR,
...
# kernel needs four accommodations, all applied here / in linux-dace:
#  - Two linux-dace kbuild patches (cmd-mod-file-func.patch,
...
modroot() {
    ...
    case "$1" in
      ...
      # for dace which is integrated/ICNSS).  CONFIG_CNSS_OUT_OF_TREE=y
...
do_install() {
    ...
    # also drag in every linux-dace-built kernel module — name conflict with
