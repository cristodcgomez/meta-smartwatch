#!/bin/sh
# Recreate the dm-linear mappings for dace's stock Android-13 partitions
...
# (modules.load.dace) where /vendor isn't visible yet, so probe silently
...
if [ -d /sys/bus/i2c/drivers/cs40l26 ] && [ -d /sys/bus/i2c/devices/0-0043 ] \
    ...
    echo 0-0043 > /sys/bus/i2c/drivers/cs40l26/bind 2>/dev/null \
        && echo "dace-vendor-mount: bound cs40l26 to 0-0043"
fi
...
# mount of mmcblk0p44 is done by dace-lxc-android-start.sh into the
...
# Mount options mirror dace-lxc-android-start.sh's firmware_mnt mount.
...
if [ -b /dev/mmcblk0p44 ] && ! mountpoint -q /vendor/firmware_mnt 2>/dev/null; then
    ...
    mount -t vfat -o ro,uid=1000,gid=1000,fmask=0337,dmask=0227 \
        ...
        && echo "dace-vendor-mount: mounted firmware_mnt (mmcblk0p44)" \
        || true
fi
...
if [ -d /sys/class/remoteproc/remoteproc1 ] \
    ...
    echo start > /sys/class/remoteproc/remoteproc1/state 2>/dev/null \
        && echo "dace-vendor-mount: kicked remoteproc-mss (modem) boot" \
        || true
fi
