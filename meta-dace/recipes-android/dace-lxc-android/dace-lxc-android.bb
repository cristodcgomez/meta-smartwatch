SUMMARY = "Dace: Halium-13 LXC container (bundle + runner)"
...
# dace-lxc-android-bundle.tar.gz holds: android-rootfs.img + config +
...
SRC_URI = "https://dl.dropboxusercontent.com/scl/fi/insvp7s5wp516iv56a4vx/dace-lxc-android.tar.gz?rlkey=xp4m8ayfwyn12nqae87o3sm3t&dl=1;downloadfilename=dace-lxc-android-bundle.tar.gz \
           file://mount-binder.sh \
           file://dace-lxc-android.service \
...
RDEPENDS:${PN} = "lxc dace-vendor-mount"
...
do_install() {
    ...
    # pm8xxx RTC, which on dace is VOLATILE and reads ~1970 -- stomping the
    ...
    install -m 0755 ${UNPACKDIR}/dace-lxc-android-start.sh \
        ${D}${libexecdir}/dace-lxc-android-start.sh
    ...
    # bluebinder ordering drop-in: see the comment in bluebinder-dace.conf.
    install -d ${D}${systemd_unitdir}/system/bluebinder.service.d
    install -m 0644 ${UNPACKDIR}/bluebinder-dace.conf \
        ${D}${systemd_unitdir}/system/bluebinder.service.d/bluebinder-dace.conf
    ...
    install -m 0644 ${UNPACKDIR}/dace-no-kmsg.conf \
        ${D}${sysconfdir}/systemd/journald.conf.d/dace-no-kmsg.conf
}
...
FILES:${PN} = "${localstatedir}/lib/lxc/android \
               ...
               ${sysconfdir}/systemd/journald.conf.d/dace-no-kmsg.conf"
